...
import http.client
import datetime
import conexionSQLServer as cS
import util as ut
...
def gradosDia(tMax, tMin):
    ...
    if (tMax > tUmbralSuperior) and (tMin > tUmbralSuperior): ## Si la temperatura máxima y mínima están por encima de 34.4.
        ...
        gradoDia=((tMax-tUmbralInferior)**2 - (tMax-tUmbralSuperior)**2)/(2*(tMax-tMin))
...
def calcularModelo(dias):
    ...
    for municipio in dfMunicipios['municipio']:
        ...
        df=df[['fecha','TMed','HrMed','PAc','RgAc','VVMed']]
        df=cS.getDatosTiempo(str(estacion))
        df=df[['Estacion','fecha','TMed','HrMed','PAc','RgAc','VVMed']]
        ...
        df['diasGrado'] = df.apply(lambda row: gradosDia(row['TMax'], row['TMin']), axis=1)
        ...
        dfMerged=dfMerged.drop(['Estacion','diasGrado','inicio_febrero'],axis=1)
